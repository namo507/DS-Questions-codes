...
# 🏋️ The Gym Metaphor: Checking for Repeat Visitors
...
For each of n members, you might check up to n names → **O(n²)** — like reading the entire guest list for every new person!
...
|:--------------------------|:---------------|:--------|
| "Is Member#5 checked in?" | Lookup in set  | O(1) ⚡ |
| "Add Member#5 to system"  | Insert to set  | O(1) ⚡ |
...
def contains_duplicate(nums: list[int]) -> bool:
    ...
    - If yes → duplicate found; If no → add them to system
...
# 🔍 Solution Breakdown: The Full Gym Session Analysis
...
**Result**: `false` — All members had unique IDs!
...
|:----------|:--------------------|:-----------|
| Lookup    | "Scan member card"  | Instant ⚡ |
| Insert    | "Register check-in" | Instant ⚡ |
...
**Best Case**: O(1) — duplicate at index 1 **Worst Case**: O(n) — no duplicates, or duplicate at end
...
| Brute Force | O(1) space | O(n²) time | Never (for interviews) |
...
For the one-liner solution: $$\text{Has Duplicates} = (\text{len}(nums) \neq \text{len}(\text{set}(nums)))$$

------------------------------------------------------------------------

*"In the gym of algorithms, the hash set is your membership card scanner — fast, reliable, and essential."* 💪🏋️